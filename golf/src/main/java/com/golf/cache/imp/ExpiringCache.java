 ...
 * This class contains a worker thread that will periodically check this class in order to determine if any objects
...
public class ExpiringCache<K, V> implements Map<K, V> {
    ...
    /**
     * Creates a new instance of ExpiringMap using the supplied time-to-live value and the default value for
     * DEFAULT_EXPIRATION_INTERVAL
     * 
     * @param timeToLive The time-to-live value (seconds)
     */
    public ExpiringCache(int timeToLive) {
        this(timeToLive, DEFAULT_EXPIRATION_INTERVAL);
    }

    /**
     * Creates a new instance of ExpiringMap using the supplied values and a {@link ConcurrentHashMap} for the internal
     * data structure.
     * 
     * @param timeToLive The time-to-live value (seconds)
     * @param expirationInterval The time between checks to see if a value should be removed (seconds)
     */
    public ExpiringCache(int timeToLive, int expirationInterval) {
        this(new ConcurrentHashMap<K, ExpiringObject>(), new CopyOnWriteArrayList<ExpirationListener<V>>(), timeToLive,
                expirationInterval);
    }

    private ExpiringCache(ConcurrentHashMap<K, ExpiringObject> delegate,
    ...
    /**
     * A Thread that monitors an {@link SafeExpiringCache} and will remove elements that have passed the threshold.
     * 
     */
 ...
 * @version $Rev: 589932 $, $Date: 2007-10-30 02:50:39 +0100 (Tue, 30 Oct 2007) $ TODO Make this a inner interface of
 *          ExpiringMap
 */
